...
async function collatz_init(path) {
  ...
  collatz_benford_impl = collwasm.collatz_benford;
...
function collatz_benford(n) {
  const len = collatz_benford_impl(n, 0);
  const result = decode_utf8(new Uint8Array(collwasm.memory.buffer, 0, len));
  const array = JSON.parse(result);

  const map = new Map();
  for (let i = 0; i < 9; i++) map.set(i + 1, array[i]);
